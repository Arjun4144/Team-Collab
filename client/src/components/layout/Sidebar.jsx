...
const WorkspaceSection = ({ ws, onRename, onDeleteChannel }) => {
  const { user, activeWorkspace, selectWorkspace, channels, showToast, generateInviteLink, createChannel, renameWorkspace, deleteWorkspace, leaveWorkspace, setConfirmModal } = useStore();
                  ...
                  <>
                    ...
                    <button onClick={() => { 
                      ...
                      setConfirmModal({
                        ...
                        message: <>Are you sure you want to delete <strong>{ws.name}</strong>? All data will be lost.</>,
                        confirmText: 'Delete',
                        type: 'danger',
                        onConfirm: async () => {
                          try { await deleteWorkspace(ws._id); showToast('Workspace deleted'); } catch { showToast('Failed'); }
                        }
                      });
                    }} style={{ ...styles.dropdownBtn, color: '#ef4444' }}>🗑️ Delete</button>
                  </>
                ) : (
                  <>
                    {isAdmin && (
                    ...
                    <button onClick={() => { 
                      ...
                      setConfirmModal({
                        ...
                        message: <>Are you sure you want to leave <strong>{ws.name}</strong>?</>,
                        confirmText: 'Leave',
                        type: 'danger',
                        onConfirm: async () => {
                          try { await leaveWorkspace(ws._id); showToast('Left workspace'); } catch { showToast('Failed'); }
                        }
                      });
                    }} style={{ ...styles.dropdownBtn, color: '#ef4444' }}>👋 Leave Workspace</button>
                  </>
                )}
              </div>
            )}
...
export default function Sidebar() {
  const { 
    user, workspaces, channels, activeWorkspace, activeChannel, 
    selectWorkspace, selectChannel, setRightPanel, rightPanel, 
    logout, showToast, renameChannel, deleteChannel, createWorkspace,
    ...
    <aside style={styles.sidebar}>
      ...
      <div style={styles.section}>
        ...
        <nav style={styles.navIcons}>
          {[{ id: 'decisions', icon: '✅', label: 'Decisions' }, { id: 'all_users', icon: '👥', label: 'People' }].map(({ id, icon, label }) => (
        ...
        >
          ...
          {profileMenuOpen && (
            <div style={{ ...styles.dropdown, bottom: '100%', left: 0, marginBottom: 8 }}>
              <button onClick={() => { setProfileUser(user); setProfileMenuOpen(false); }} style={styles.dropdownBtn}>⚙️ Profile Settings</button>
...
const styles = {
  ...
  wsIcon: { width: 34, height: 34, borderRadius: 8, background: 'var(--accent)', color: 'var(--text-inverse)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, fontWeight: 700, fontFamily: 'var(--font-display)', flexShrink: 0 },
  ...
  channelActive: { background: 'var(--bg-active)', color: 'var(--text-primary)', fontWeight: 500 },
  ...
  wsIcon2: { width: 22, height: 22, borderRadius: '50%', background: 'var(--accent-dim)', color: 'var(--text-inverse)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 600, flexShrink: 0 },
  wsLabel2: { fontSize: 14, fontWeight: 500, color: 'var(--text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' },
  unreadDot: { width: 8, height: 8, borderRadius: '50%', background: 'var(--intent-action)', flexShrink: 0, marginLeft: 4 },
  ...
  unreadBadge: { background: 'var(--intent-action)', color: '#fff', fontSize: 10, fontWeight: 700, padding: '2px 6px', borderRadius: 10, flexShrink: 0, lineHeight: 1 },
  ...
  modalDeleteBtn: { padding: '8px 16px', borderRadius: 6, background: 'var(--intent-action)', color: '#fff', border: 'none', cursor: 'pointer', fontWeight: 600 }
};
